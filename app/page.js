...
export default function CaroControl() {
  ...
  const createItem = useCallback(async (table, item, setter, list) => {
    ...
    setSaving(false);
    if (r.ok) { setter([r.data, ...list]); showToast("success", "Guardado"); return true; }
  ...
  const updateItem = useCallback(async (table, id, item, setter, list) => {
    ...
    setSaving(false);
    if (r.ok) { setter(list.map((x) => (x.id === id ? r.data : x))); showToast("success", "Guardado"); return true; }
    showToast("error", `No se pudo guardar: ${r.error}`);
    return false;
  }, [getValidToken, showToast]);
  ...
  const deleteItem = useCallback(async (table, id, setter, list) => {
    ...
    setSaving(false);
    if (r.ok) { setter(list.filter((x) => x.id !== id)); showToast("success", "Eliminado"); return true; }
...
/* ============================================================
   Nota rápida (esqueleto — procesamiento manual por ahora)
   ============================================================ */
...
function NotaRapidaView({ notas, onGuardar, onMarcarProcesada, onEliminar }) {
  ...
  return (
    <div>
      <SectionTitle sub="Por ahora el procesamiento es manual: escribes tu nota, la llevas a Claude.ai para estructurarla, y creas los elementos (tarea, cobro, cita) desde sus módulos. Cuando conectemos la API, este paso se vuelve automático.">
        Nota rápida
      </SectionTitle>
      ...
      <Card className="p-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-3">
          <button
            ...
            style={{ color: "#7A2E4A", border: "1px solid #7A2E4A" }}
          >
            <Sparkles size={14} /> {copiado ? "Prompt copiado" : "Copiar prompt para Claude.ai"}
          </button>
          <button
      ...
      {notas.length === 0 ? (
        <EmptyState icon={PenLine} text="Tus notas rápidas guardadas aparecerán aquí." />
      ) : (
        <div className="space-y-2">
          {notas.map((n) => (
